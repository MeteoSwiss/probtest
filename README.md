...
Objective: Run the mch_opr_r04b07_lhn_12 ICON experiment and check if the output of the run is ok. Probtest requires some additional python packages. On daint, there is a pre-installed python environment which can be loaded with:
...
```
probtest.py init --codebase-install /path/to/the/ICON/Installation/ --experiment-name mch_opr_r04b07_lhn_12 --reference /path/to/icon-test-references/daint_cpu_pgi/ --file-ids atm_3d_ml,atm_3d_hl,atm_3d_pl --time_dim time
```
...
- `tolerance/mch_opr_r04b07_lhn_12.csv`: contains tolerance ranges computed from the stats-files

These can then be used to compare against the output of a test binary (usually a GPU binary). For that, manually run the `exp.mch_opr_r04b07_lhn_12.run` experiment with the test binary to produce the test output. Then use probtest to generate the stats file for this output:

```
python probtest.py stats --model-output-dir /path/to/test-icon/experiments/mch_opr_r04b07_lhn_12 --stats-file-name stats_cur.csv
```
...
Now all files needed to perform a probtest check are available; the reference file `stats_ref.csv`, the test file `stats_cur.csv` as well as the tolerance range `tolerance/mch_opr_r04b07_lhn_12.csv`. Providing these files to `check` will perform the check:
...
Download the probtest input data (generated by ICON/probtest), then run the initialization script to generate the associated config file from the template:
    ...
    scp -r daint:/project/g110/probtest_testdata $REFERENCE_DATA
    python probtest.py init --codebase-install $REFERENCE_DATA/icon_data --reference $PROBTEST_CUR_DATA --template-name templates/testdata.jinja --experiment-name atm_amip_test --config testdata.json --file-ids=atm_3d,lnd --member_ids=1,2 --time_dim time
...
`PROBTEST_TEST_EXPERIMENT`: full sets of reference data are generated for both the `mch_opr_r04b07_lhn_12` and `atm_amip_test` experiments. Chose one of the two or run both sequentially.
