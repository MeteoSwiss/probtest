...
### Example: Check the output of an ICON experiment

Objective: Run the mch_opr_r04b07 ICON experiment and check if the output of the run is ok.

All requirements for using probtest can be easily installed with conda using the setup scripts:
...
Initialized a `probtest.json` file in your cpu subdir, `exp_name` here should
refer to your experiment script:
...
Now you should have created a `probtest.json` file in the current directory.
This file contains all information needed by probtest to process the ICON experiment.

#### Generate references and tolerances on CPU
With everything set up properly, the chain of commands can be invoked to run the
CPU reference binary (`run-ensemble`), generate the statistics files used for
probtest comparisons (`stats`) and generate tolerances from these files
(`tolerance`).
Then run the perturbed experiments and wait for the submitted jobs to finish:
```console
...
FYI: if your experiment you experiment does not generate all of the files listed
in the `file-id`s above, you you receive a message that certain `file-id`
patters do not match any file.
Those files can remove them from `file-id`s.

Finally create your non-standard test tolerance.csv file by analysing those statistics:
```console
python ../externals/probtest/probtest.py tolerance
```

Note the `--ensemble` option which is set to take precedence over the default
`False` from the configuration and make probtest process the model output from
each ensemble generated by `run-ensemble`.
