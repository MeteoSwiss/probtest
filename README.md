...
```
probtest.py init --codebase-install /path/to/the/ICON/Installation/ --experiment-name mch_opr_r04b07_lhn_12 --reference /path/to/icon-test-references/daint_cpu_pgi/ --file-ids "*atm_3d_ml*,*atm_3d_hl*,*atm_3d_pl*""
```
...
Download the probtest input data (generated by ICON/probtest), then run the initialization script to generate the associated config file from the template:
    ...
    python probtest.py init --codebase-install $REFERENCE_DATA/icon_data --reference $PROBTEST_CUR_DATA --template-name templates/testdata.jinja --experiment-name atm_amip_test --config testdata.json "--file-ids=*atm_3d*,*lnd*" --member_ids=1,2
