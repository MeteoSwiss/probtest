...
Uses all given stats files generated for a model ensemble with `stats`. From those stats files, randomly selects up to a given number of members to generate the tolerances with `tolerance`. Repeats this process until finding a selection for which all other members pass the tolerance `check`. This selection is then considered optimal.
