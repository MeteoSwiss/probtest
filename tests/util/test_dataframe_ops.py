...
from util.dataframe_ops import parse_check, parse_probtest_csv
...
def setup_csv_files(tmp_path):
    ...
    tolerance_data = pd.DataFrame({
        'A': [0.1, 0.2],
        'B': [0.3, 0.4]
    }, index=pd.MultiIndex.from_tuples([('a', 'b'), ('c', 'd')], names=['col1', 'col2']))
    ref_data = pd.DataFrame({
        'A': [1, 2],
        'B': [3, 4]
    }, index=pd.MultiIndex.from_tuples([('a', 'b', 'c'), ('d', 'e', 'f')], names=['col1', 'col2', 'col3']))
...
def test_parse_check(mock_logger, mock_parse_probtest_csv, setup_csv_files):
    ...
    mock_parse_probtest_csv.side_effect = lambda file, index_col: pd.read_csv(file, index_col=index_col)
